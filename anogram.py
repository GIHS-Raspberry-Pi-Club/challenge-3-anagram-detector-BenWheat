...
def anogram_check(string_1,string_2):
    ...
    for i in no_punc_str1: #for statements goes through each letter in first phrase
        if i in no_punc_str2: #if letter iis in first phrase and second phrase
        ...
        print(no_punc_str2)
    if no_punc_str1 == no_punc_str2: #if bot phrase are equal to each other(they are also equal to nothing)
        print(string_1, 'and', string_2, 'are anograms') #tell user phrase are anograms
    else:
